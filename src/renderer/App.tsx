...
export default function App() {
    ...
    const sysAudioCtxRef = useRef<any>(null);
    ...
    const startSystemAudioCapture = async () => {
        ...
        try {
            ...
            // Use MediaRecorder to encode system audio as WebM directly (Opus format).
            // This natively syncs audio frames using exact system clocks and avoids ALL custom PCM/IPC bugs.
            const mediaRecorder = new MediaRecorder(stream, { mimeType: 'audio/webm;codecs=opus' });
            sysAudioCtxRef.current = mediaRecorder; // Using the ref just to hold and stop it later

            mediaRecorder.ondataavailable = async (e) => {
                if (e.data.size > 0 && window.api) {
                    const arrayBuffer = await e.data.arrayBuffer();
                    window.api.sendSystemAudioData(arrayBuffer);
                }
            };

            mediaRecorder.start(100); // Process tiny chunks to reduce IPC spike-loads
            console.log('[SysAudio] Capture started with WebM MediaRecorder.');
        } catch (err) {
